...
describe('AI Guardrails Generator', () => {
  ...
  describe('setupAIGuardrails', () => {
    ...
    it('generates SKILL.md with correct structure', async () => {
      ...
      const skillWriteCall = vi.mocked(writeFileSync).mock.calls.find(
        (call) => String(call[0]).includes('SKILL.md')
      );
  ...
  describe('token files', () => {
    it('creates colors.md with token table', async () => {
      ...
      const colorsWriteCall = vi.mocked(writeFileSync).mock.calls.find(
        (call) => String(call[0]).includes('colors.md')
      );
    ...
    it('creates spacing.md with token table', async () => {
      ...
      const spacingWriteCall = vi.mocked(writeFileSync).mock.calls.find(
        (call) => String(call[0]).includes('spacing.md')
      );
